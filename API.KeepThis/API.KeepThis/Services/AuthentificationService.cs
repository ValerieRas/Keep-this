...
namespace API.KeepThis.Services
{
    public class AuthentificationService : IAuthentificationService
    {
        ...
        public AuthentificationService(IConfiguration config, IPasswordSecurity passwordSecurity, IUsersRepository UsersRepository, IAuthTokenRepository authTokenRepository)
        {
            _secretKey = config.GetConnectionString("ConnectDB");
        ...
        public async Task<LoginUserDto> AuthenticateUserAsync(LoginDto request)
        {
            ...
            var userToken = new AuthentificationToken
            {
                IdUser = user.IdUser,
                Token = token,
                TimestampToken = DateTime.Now.AddMonths(2)
            };

            var authToken= _tokenRepository.GetTokenByUserIdAsync(user.IdUser);

            if (authToken != null)
            {
                await _tokenRepository.RemoveTokenAsync(authToken.Result);
            }
